...
export default function DashboardPage() {
  ...
  const loadData = () => {
    setLoading(true);
    Promise.all([
      supabase.from("media").select("*, kit:kits(*)").order("created_at", { ascending: false }).limit(200),
  ...
  const filteredMedia = useMemo(() => {
    ...
    return list;
  }, [allMedia, selectedKits, selectedTags, selectedWeapons, idFilter, sortDir]);
              ...
              {(kitsWithMedia.length > 0 || hasUnassigned) && (
                <div className="mb-3">
                  ...
                  >
                    ...
                    Kits
                    {selectedKits.size > 0 && <span className="ml-auto text-accent">{selectedKits.size}</span>}
                  </button>
                  {showKits && (
                    <div className="space-y-0.5">
                      {hasUnassigned && (
                        <button
                          ...
                          className={clsx(
                            ...
                            selectedKits.has("__unassigned__") ? "bg-accent/15 text-accent" : "text-text-secondary hover:bg-white/5"
                          )}
                        >
                          <span className="italic">Unassigned</span>
                          <span className="text-[9px] opacity-60">{allMedia.filter((m) => !m.kit_id).length}</span>
                        </button>
                      )}
                      {kitsWithMedia.map((kit) => (
                        <button
                          ...
                          className={clsx(
                            "w-full flex items-center justify-between px-2 py-1 text-[11px] font-mono rounded-sm transition-colors",
                            selectedKits.has(kit.id) ? "bg-accent/15 text-accent" : "text-text-secondary hover:bg-white/5"
                          )}
                        >
                          <span className="truncate">{kit.name}</span>
                          <span className="text-[9px] opacity-60 shrink-0 ml-1">{allMedia.filter((m) => m.kit_id === kit.id).length}</span>
                        </button>
                      ))}
                    </div>
                  )}
